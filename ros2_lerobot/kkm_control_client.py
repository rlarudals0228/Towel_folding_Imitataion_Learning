...
class InferenceACT(Node):
    def __init__(self):
        ...
        if not self.configs:
            self.get_logger().error("No valid config file found.")
            sys.exit(1)

        # 기본 config는 flatten으로 시작
        self.config = self.configs.get('flatten')
    ...
    def decision_callback(self, msg):
        try:
            ...
            if decision != self.last_decision:
                self.get_logger().info(f"Decision changed from {self.last_decision} to {decision}")
                self.last_decision = decision
                
            self.observation_flag['camera'] = True
        except Exception as e:
            self.get_logger().error(f"Decision callback error: {e}")
            self.observation_flag['camera'] = False    
            
    def send_request(self):
        if self.cli is None:
            self.get_logger().error(":x: No inference service client set yet.")
            return
    ...
    def follower_joint_callback(self, msg):
        try:
            ...
            if self.check_step_completion() and not self.move_init_positions:
                self.get_logger().info('Success to move initial positions!')
    ...
    def keyboard_callback(self, msg):
        try:
            if msg.data == 'flatten':
                self.cli = self.create_client(Inference, 'run_inference_towel_flattening')
                while not self.cli.wait_for_service(timeout_sec=1.0):
                    self.get_logger().info('Waiting for FLATTENING service...')
                self.start_episod = True
                self.get_logger().info("⌨️ [KEYBOARD] Flattening mode ON")
                self.send_request()
                self.response_callback(update_current=True)

            elif msg.data == 'fold':
                self.cli = self.create_client(Inference, 'run_inference_towel_folding')
                while not self.cli.wait_for_service(timeout_sec=1.0):
                    self.get_logger().info('Waiting for FOLDING service...')
                self.start_episod = True
                self.get_logger().info("⌨️ [KEYBOARD] Folding mode ON")

            elif msg.data == 'shoot':
                if self.shoot_triggered:
                    self.get_logger().info("⌨️ [KEYBOARD] Shoot already triggered.")
                    return
                
                if self.last_decision == "flatten":
                    self.cli = self.create_client(Inference, 'run_inference_towel_flattening')
                    while not self.cli.wait_for_service(timeout_sec=1.0):
                        self.get_logger().info('Waiting for FLATTENING service...')
                    self.start_episod = True
                    self.get_logger().info("⌨️ [Shoot] Shoot Flattening mode ON")
                    self.send_request()
                    self.response_callback(update_current=True)
                    
                elif self.last_decision == "fold":
                    self.cli = self.create_client(Inference, 'run_inference_towel_folding')
                    while not self.cli.wait_for_service(timeout_sec=1.0):
                        self.get_logger().info('Waiting for FOLDING service...')
                    self.start_episod = True
                    self.get_logger().info("⌨️ [Shoot] Shoot Folding mode ON")
                    self.send_request()
                    self.response_callback(update_current=True)
            
                else:
                    self.get_logger().warn(" No Decision available from camera yet.")

                #self.shoot_triggered = True
            
            elif msg.data == 'start':
                self.start_episod = True
                self.get_logger().info("⌨️ [KEYBOARD] Start episode")
    ...
    def pub_interpolation_action(self):
        ...
        if 30 * self.cnt_timer > 400 * self.index_action:
            ...
            if self.enable_temporal_ensemble:
                ...
                x_new = self.cur_actions[min(self.current_chunk_index, self.n_action_step - 1)].copy()
        ...
        if not self.enable_temporal_ensemble:
            if self.enable_spline:
                ...
                if self.spline == 'cubic':
                    ...
                    spline_action = quintic_spline_multi(t, t_0, t_f,
                                                        x_0, x_dot_0, x_ddot_0,
                                                        x_f, x_dot_f, x_ddot_f)[0]
    ...
    def check_step_completion(self):
        return all(abs(curr - target) < self.epsilon
